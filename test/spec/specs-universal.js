...
describe('landy.js', function() {
  ...
  describe('startLandy', function() {
    ...
    it('should initialize Landy campaign on correct url', function() {
      ...
      spyOn(window, 'Landy').and.callFake(function(campaignId, url, type, subtype, goals) {
        landyObj = new OriginalConstructor(campaignId, url, type, subtype, goals);
        spyOn(landyObj, 'init').and.callThrough();
        return landyObj;
      });
      startLandy(campaignsWithValidUrl);
    ...
    it('should initialize Landy campaign on correct goal url', function() {
      ...
      spyOn(window, 'Landy').and.callFake(function(campaignId, url, type, subtype, goals) {
        landyObj = new OriginalConstructor(campaignId, url, type, subtype, goals);
        spyOn(landyObj, 'sendSuccess').and.callThrough();
        return landyObj;
      });
      startLandy(campaignsWithValidGoal);
      expect(landyObj.sendSuccess).toHaveBeenCalled();
    });
  });
